...
public class PlaylistServiceImpl extends ServiceImpl<PlaylistMapper, Playlist> implements IPlaylistService {
        ...
        List<PlaylistVO> playlistVOList = playlistPage.getRecords().stream()
                .map(playlist -> {
                    ...
                    if (playlistVO.getCoverUrl() != null) {
                        playlistVO.setCoverUrl(playlistVO.getCoverUrl().replace("localhost:9000", "127.0.0.1:9001"));
                    }
                    return playlistVO;
                }).toList();
        ...
        if (userId == null) {
            ...
            playlists.forEach(playlist -> {
                ...
                if (playlist.getCoverUrl() != null) {
                    playlist.setCoverUrl(playlist.getCoverUrl().replace("localhost:9000", "127.0.0.1:9001"));
                }
            });
    ...
    public Result<PlaylistDetailVO> getPlaylistDetail(Long playlistId, HttpServletRequest request) {
        ...
        // 强制转换 URL 端口（歌单封面）
        if (playlistDetailVO.getCoverUrl() != null) {
            playlistDetailVO.setCoverUrl(playlistDetailVO.getCoverUrl().replace("localhost:9000", "127.0.0.1:9001"));
        }

        // 设置默认状态，同时强制转换歌曲 URL 端口
