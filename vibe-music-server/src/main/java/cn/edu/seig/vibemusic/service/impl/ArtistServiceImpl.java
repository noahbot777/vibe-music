...
public class ArtistServiceImpl extends ServiceImpl<ArtistMapper, Artist> implements IArtistService {
        ...
        List<ArtistVO> artistVOList = artistPage.getRecords().stream()
                .map(artist -> {
                    ...
                    if (artistVO.getAvatar() != null) {
                        artistVO.setAvatar(artistVO.getAvatar().replace("localhost:9000", "127.0.0.1:9001"));
                    }
                    return artistVO;
                }).toList();
    ...
    public Result<ArtistDetailVO> getArtistDetail(Long artistId, HttpServletRequest request) {
        ...
        if (artistDetailVO.getAvatar() != null) {
            artistDetailVO.setAvatar(artistDetailVO.getAvatar().replace("localhost:9000", "127.0.0.1:9001"));
        }
