...
import cn.edu.seig.vibemusic.model.vo.SongUrlVO;
...
public class SongServiceImpl extends ServiceImpl<SongMapper, Song> implements ISongService {
    ...
    public Result<PageResult<SongVO>> getAllSongs(SongDTO songDTO, HttpServletRequest request) {
        ...
        // 强制转换 URL 端口
        songVOList.forEach(this::convertSongVOUrl);

        return Result.success(new PageResult<>(songPage.getTotal(), songVOList));
    }

    /**
     * 转换歌曲 URL 端口（强制从 9000 改为 9001）
     */
    private void convertSongVOUrl(SongVO songVO) {
        if (songVO.getCoverUrl() != null) {
            songVO.setCoverUrl(songVO.getCoverUrl().replace("localhost:9000", "127.0.0.1:9001"));
        }
        if (songVO.getAudioUrl() != null) {
            songVO.setAudioUrl(songVO.getAudioUrl().replace("localhost:9000", "127.0.0.1:9001"));
        }
    }
    ...
    public Result<List<SongVO>> getRecommendedSongs(HttpServletRequest request) {
        ...
        // 用户未登录，返回随机歌曲列表
        if (map == null) {
            List<SongVO> songVOs = songMapper.getRandomSongsWithArtist();
            songVOs.forEach(this::convertSongVOUrl);
            return Result.success(songVOs);
        }
        ...
        // 强制转换 URL 端口
        recommendedSongs.forEach(this::convertSongVOUrl);
    ...
    public Result<SongDetailVO> getSongDetail(Long songId, HttpServletRequest request) {
        ...
        if (songDetailVO.getCoverUrl() != null) {
            songDetailVO.setCoverUrl(songDetailVO.getCoverUrl().replace("localhost:9000", "127.0.0.1:9001"));
        }
        if (songDetailVO.getAudioUrl() != null) {
            songDetailVO.setAudioUrl(songDetailVO.getAudioUrl().replace("localhost:9000", "127.0.0.1:9001"));
        }
    ...
    @Override
    public Result<List<SongUrlVO>> getSongUrl(Long songId, String level) {
        Song song = songMapper.selectById(songId);
        if (song == null) {
            return Result.error(MessageConstant.SONG + MessageConstant.NOT_FOUND);
        }
        
        List<SongUrlVO> urlList = new ArrayList<>();
        SongUrlVO vo = new SongUrlVO();
        vo.setId(songId);
        // 强制转换 URL 端口
        String audioUrl = song.getAudioUrl();
        if (audioUrl != null) {
            audioUrl = audioUrl.replace("localhost:9000", "127.0.0.1:9001");
        }
        vo.setUrl(audioUrl);
        urlList.add(vo);
